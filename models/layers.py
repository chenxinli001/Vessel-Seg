...
class expand_compress_block3_(nn.Module):
    def __init__(self, n_in):
        super().__init__()
...
class output_block2_xunit(nn.Module):
    def __init__(self, n_in):
        super().__init__()

        self.conv1 = nn.Conv2d(n_in, 32, kernel_size=3, padding=1)    
        self.bn_xUnit = nn.BatchNorm2d(32)      
    ...
    def forward(self, x_in):
        x1 = self.conv1(x_in)
        x_xUnit = self.conv2(F.relu(self.bn_xUnit(x1)))
        output1 = self.bn_out1(self.conv_out1(x1))
        
        weight = F.relu(output1)
...
class output_block2_weight(nn.Module):
    def __init__(self, n_in):
        super().__init__()
...
#        weight = F.threshold(1-weight,0.5,torch.exp(-torch.mul(weight,weight)))
        ...
        sigma = 1
        weight = sigma*torch.exp(-torch.mul(weight,weight)) + 1 - sigma*torch.exp(-ones.cuda()/4)
        #weight = torch.exp(-torch.mul(weight,weight))
