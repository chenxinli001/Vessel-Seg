...
import natsort
from Tools.AVclassifiationMetrics import AVclassifiationMetrics
import natsort
...
def get_patch_trad(batch_size, patch_size, train_data1_, train_data2_,train_data3_,label_data1,train_data4_, train_data5_,train_data6_,label_data2):
    ...
    for j in range(batch_size):
        ...
        data_choice = np.random.randint(0,A+B)
        if data_choice >= A-1:
...
def modelEvalution(i,net,savePath,loss_all):
    ...
    ArteryAUC,ArteryAcc,ArterySp,ArterySe,VeinAUC,VeinAcc,VeinSp,VeinSe = Evalution_AV(ArteryPredAll,VeinPredAll,VesselPredAll,LabelArteryAll,LabelVeinAll,LabelVesselAll,MaskAll,0)
    ...
    ArteryAUC,ArteryAcc,ArterySp,ArterySe,VeinAUC,VeinAcc,VeinSp,VeinSe = Evalution_AV(ArteryPredAll,VeinPredAll,VesselPredAll,LabelArteryAll,LabelVeinAll,LabelVesselAll,MaskAll,1)
    #VeinAUC,VeinAcc,VeinSp,VeinSe = Evalution(VeinPredAll,LabelVeinAll, MaskAll)
    VesselAUC,VesselAcc,VesselSp,VesselSe = Evalution(VesselPredAll,LabelVesselAll, MaskAll)
...
#    print('Enlarged shape:', Img_enlarged.shape)
    ...
    Net = ResUnet_illum_tran_ds_add_conv4_sSE_cSE_up(resnet='resnet18',  num_classes= n_classes)
    ...
    for i in range(max_iter):
        ...
        output_temp,_1,_2 = Net(patches_input_temp1,patches_input_temp2,patches_input_temp3,patches_input_temp4,patches_input_temp5)
