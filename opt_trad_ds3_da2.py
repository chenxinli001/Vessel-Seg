# -*- coding: utf-8 -*-
"""
Created on Fri Jan 18 20:55:07 2019
...
import natsort
from Tools.AVclassifiationMetrics import AVclassifiationMetrics,AVclassifiationMetrics_skeletonPixles
import natsort
...
def get_patch_trad(batch_size, patch_size, train_data1_, train_data2_,train_data3_,label_data1,train_data4_, train_data5_,train_data6_,label_data2):
    ...
    A =  (train_data1_.shape[2]-patch_size+1)*(train_data1_.shape[3]-patch_size+1)*train_data1_.shape[0]
    B =  (train_data4_.shape[2]-patch_size+1)*(train_data4_.shape[3]-patch_size+1)*train_data4_.shape[0]
    
    for j in range(batch_size):
        
        data_choice = np.random.randint(0,5)

        if data_choice >= 1:
            train_data1 = train_data1_
            train_data2 = train_data2_
            train_data3 = train_data3_
            label_data = label_data1
            len1 = 96
            len2 = 128
        else:
            train_data1 = train_data4_
            train_data2 = train_data5_
            train_data3 = train_data6_
            label_data = label_data2
            len1 = 96
            len2 = 128
        ...
        z = np.random.randint(0,train_data1.shape[0])
        ...
        if random_size == 0:
            ...
            data_mat_3 = np.zeros((3, patch_size, patch_size), np.float32)
            data_mat_1 = np.transpose(cv2.resize(np.transpose(train_data1[z,:,x:x+len1,y:y+len1],(1,2,0)), (patch_size,patch_size)),(2,0,1))
            data_mat_2 = np.transpose(cv2.resize(np.transpose(train_data2[z,:,x:x+len1,y:y+len1],(1,2,0)), (patch_size,patch_size)),(2,0,1))
            data_mat_3[0:2,:,:] = np.transpose(cv2.resize(np.transpose(train_data3[z,:,x:x+len1,y:y+len1],(1,2,0)), (patch_size,patch_size)),(2,0,1))
            label_mat = np.transpose(cv2.resize(np.transpose(label_data[z,:,x:x+len1,y:y+len1],(1,2,0)), (patch_size,patch_size)),(2,0,1)) 
        else:
            data_mat_3 = np.zeros((3, patch_size, patch_size), np.float32)
            data_mat_1 = np.transpose(cv2.resize(np.transpose(train_data1[z,:,x:x+len2,y:y+len2],(1,2,0)), (patch_size,patch_size)),(2,0,1))
            data_mat_2 = np.transpose(cv2.resize(np.transpose(train_data2[z,:,x:x+len2,y:y+len2],(1,2,0)), (patch_size,patch_size)),(2,0,1))
            data_mat_3[0:2,:,:] = np.transpose(cv2.resize(np.transpose(train_data3[z,:,x:x+len2,y:y+len2],(1,2,0)), (patch_size,patch_size)),(2,0,1))
            label_mat = np.transpose(cv2.resize(np.transpose(label_data[z,:,x:x+len2,y:y+len2],(1,2,0)), (patch_size,patch_size)),(2,0,1))
...
def modelEvalution(i,net,savePath,loss_all):
    ...
    for k in range(20):
        ...
        #Mask  = GetMask(k+1)
        MaskAll[k,:,:,:] = Mask
        cv2.imwrite("./data/save_image/Artery/"+str(k)+".png",ArteryPred[0]*255)
        cv2.imwrite("./data/save_image/Vein/"+str(k)+".png",VeinPred[0]*255)
        cv2.imwrite("./data/save_image/Vessel/"+str(k)+".png",VesselPred[0]*255)
        cv2.imwrite("./data/save_image/Mask0/"+str(k)+".png",Mask[0]*255)
    ...
    np.save("./ProMap3.npy",ProMap)
    np.save("./Label0.npy",LabelMap)
    np.save("./Mask0.npy",MaskAll)
    ArteryAUC,ArteryAcc,ArterySp,ArterySe,VeinAUC,VeinAcc,VeinSp,VeinSe = Evalution_AV_skeletonPixles(ArteryPredAll,VeinPredAll,VesselPredAll,LabelArteryAll,LabelVeinAll,LabelVesselAll,MaskAll,0)
    #VeinAUC,VeinAcc,VeinSp,VeinSe = Evalution(VeinPredAll,LabelVeinAll, MaskAll)
    VesselAUC,VesselAcc,VesselSp,VesselSe = Evalution_drive(VesselPredAll)
    ...
    ArteryAUC,ArteryAcc,ArterySp,ArterySe,VeinAUC,VeinAcc,VeinSp,VeinSe = Evalution_AV_skeletonPixles(ArteryPredAll,VeinPredAll,VesselPredAll,LabelArteryAll,LabelVeinAll,LabelVesselAll,MaskAll,1)
    #VeinAUC,VeinAcc,VeinSp,VeinSe = Evalution(VeinPredAll,LabelVeinAll, MaskAll)
...
def Evalution_AV(PredAll1,PredAll2,VesselPredAll,LabelAll1,LabelAll2, LabelVesselAll,MaskAll,DataSet=0):
    threshold_confusion = 0.5
    y_scores1, y_true1,y_scores2, y_true2 = pred_only_FOV_AV(PredAll1,PredAll2,LabelAll1,LabelAll2, MaskAll,threshold_confusion)
    AUC1 = roc_auc_score(y_true1,y_scores1)   
    AUC2 = roc_auc_score(y_true2,y_scores2)            
    accuracy1,specificity1,sensitivity1 = AVclassifiationMetrics(PredAll1,PredAll2,VesselPredAll,LabelAll1,LabelAll2,LabelVesselAll,DataSet)  
...
def Evalution_AV_skeletonPixles(PredAll1,PredAll2,VesselPredAll,LabelAll1,LabelAll2, LabelVesselAll,MaskAll,DataSet=0):
    threshold_confusion = 0.5
    y_scores1, y_true1,y_scores2, y_true2 = pred_only_FOV_AV(PredAll1,PredAll2,LabelAll1,LabelAll2, MaskAll,threshold_confusion)
    AUC1 = roc_auc_score(y_true1,y_scores1)   
    AUC2 = roc_auc_score(y_true2,y_scores2)            
    accuracy1,specificity1,sensitivity1 = AVclassifiationMetrics_skeletonPixles(PredAll1,PredAll2,VesselPredAll,LabelAll1,LabelAll2,LabelVesselAll,DataSet)  
    #accuracy2,specificity2,sensitivity2 = AVclassifiationMetrics(PredAll2,PredAll1,VesselPredAll,LabelAll2,LabelAll1,LabelVesselAll,DataSet)      
...
def GetResult(net,k,Vessel):
    if k<=9:
        ...
        Mask[Mask0[:,:,2]>0] = 1
        #VesselLabelName ='./data/AV_DRIVE/test/1st_manual/0' + str(k) + '_manual1.gif' 
    elif k<=20:
        ...
        Mask = np.zeros((Mask0.shape[0],Mask0.shape[1]),np.float32)
        Mask[Mask0[:,:,2]>0] = 1
        #VesselLabelName ='./data/AV_DRIVE/test/1st_manual/' + str(k) + '_manual1.gif' 
    else:
...
#        LabelName = './data/HRF_RevisedAVLabel/Label/image' + str(k+10) + '.jpg'
    ...
    Img0 = cv2.imread(ImgName)
    #VesselLabel = cv2.imread(VesselLabelName)
...
#    print('Enlarged shape:', Img_enlarged.shape)
    ...
    Net = ResUnet_illum_tran_ds3_add_conv5_sSE_cSE_up11(resnet='resnet18',  num_classes= n_classes)
    ...
    for i in range(max_iter):
        ...
        output_temp,_1,_2,_3 = Net(patches_input_temp1,patches_input_temp2,patches_input_temp3,patches_input_temp4,patches_input_temp5)
...
def DataSet_random_illum(ImgPath,LabelPath): 
...
#    LabelPath = path + "av/"
    ...
    for i in range(0,len(ImgDir)):
        ...
        Img0 = cv2.imread(ImgPath + ImgDir[i])
        Label0 = cv2.imread(LabelPath + LabelDir[i])
