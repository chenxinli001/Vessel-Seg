...
def VesselProMap(path,Mask):
    ...
    ImgList = [x  for x in ImgList0 if x.__contains__('.jpg') or x.__contains__('.JPG') or x.__contains__('.tif')]
    ...
    ImgResized = imageResize(Img0, downsizeRatio=downsizeRatio)
    ...
    for i in range(len(ImgList)):
        ...
        downsizeRatio = 1000./np.maximum(Img0.shape[0], Img0.shape[1])
        ImgResized = imageResize(Img0, downsizeRatio=downsizeRatio)
        MaskResized = imageResize(TempMask, downsizeRatio=downsizeRatio)
